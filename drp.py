...
def handle_activity(view, is_write=False, idle=False):
    ...
    format_dict = dict(
        ...
        currentline=view.rowcol(view.sel()[0].begin())[0] + 1,
    )
    last_file = entity
    last_edit = time.time()
...
def handle_error(exc, retry=True):
    sublime.active_window().status_message("[dsRP] Sending activity failed")
...
def connect(silent=False, retry=True):
    ...
    try:
        ...
        if not silent:
            sublime.error_message("[dsRP] Unable to connect to Discord client."
